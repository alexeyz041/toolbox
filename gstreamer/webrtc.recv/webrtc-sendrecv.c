...
static gchar *our_id = "1";
static const gchar *server_url = "ws://127.0.0.1:57778/ws";
...
*/
    ...
    pipe1 = gst_parse_launch ("webrtcbin name=webrtcbin stun-server=stun://"
      STUN_SERVER " "
...
*/
  ...
  g_signal_emit_by_name(webrtc1, "add-transceiver", GST_WEBRTC_RTP_TRANSCEIVER_DIRECTION_RECVONLY, video_caps, NULL, &trans);
...
//      gchar *data_string;
      ...
      root_json = json_parser_get_root(parser);
      ...
      if (!json_object_has_member (root_json_object, "data")) {
        gst_printerr ("Received message without data field\n");
        goto err;
      }
...
//        GstPromise *promise;
        ...
        if (!json_object_has_member (data_json_object, "type")) {
          gst_printerr("Received SDP message without type field\n");
          goto err;
        }
        ...
        if (!json_object_has_member (data_json_object, "sdp")) {
          gst_printerr("Received SDP message without SDP string\n");
          goto err;
        }
        ...
        if (!json_object_has_member (data_json_object, "sdpMLineIndex")) {
          gst_printerr("Received ICE message without mline index\n");
          goto err;
        }
        mline_index = json_object_get_int_member (data_json_object, "sdpMLineIndex");
        if (!json_object_has_member (data_json_object, "candidate")) {
           gst_printerr("Received ICE message without ICE candidate string\n");
           goto err;
        }
        ...
        if(candidate_string) {
          g_signal_emit_by_name (webrtc1, "add-ice-candidate",  mline_index, candidate_string);
        }
      } else {
err:

//
// based on https://github.com/angrycoding/sps-parser
//
...
{
    size_t size = 0;
    uint8_t *buffer = load(argv[1], &size);
    if(!buffer) {
    ...
    if(buffer[0] != 0 || buffer[1] != 0 || buffer[2] != 0 || buffer[3] != 1 || ((buffer[4] & 0x1f) != 7)) {
        printf("this is no SPS\n");
        free(buffer);
    }
    uint32_t dimensions = sps_parser(buffer+5);
    free(buffer);

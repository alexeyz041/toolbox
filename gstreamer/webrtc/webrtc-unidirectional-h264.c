...
void soup_http_handler (SoupServer * soup_server, SoupMessage * message, const char *path, GHashTable * query, SoupClientContext * client_context,
    gpointer user_data);
void soup_websocket_handler (G_GNUC_UNUSED SoupServer * server, SoupWebsocketConnection * connection, const char *path,
...
//  GstWebRTCRTPTransceiver *trans;
  ...
  if(input_file) {
      char p[1024] = {0};
      snprintf(p, sizeof(p), "webrtcbin name=webrtcbin stun-server=stun://" STUN_SERVER " "
      "filesrc location=%s ! qtdemux ! h264parse ! "
      "rtph264pay config-interval=-1 name=payloader ! "
      "application/x-rtp,media=video,encoding-name=H264,payload=" RTP_PAYLOAD_TYPE " ! webrtcbin. ", input_file);
      receiver_entry->pipeline = gst_parse_launch (p, &error);
  } else {
    receiver_entry->pipeline =
      gst_parse_launch ("webrtcbin name=webrtcbin stun-server=stun://" STUN_SERVER " "
      VIDEO_SRC
      " ! videorate ! videoscale ! video/x-raw,width=800,height=600,framerate=25/1 ! videoconvert ! queue max-size-buffers=1 ! x264enc bitrate=600 speed-preset=ultrafast tune=zerolatency key-int-max=15 ! video/x-h264,profile=constrained-baseline ! queue max-size-time=100000000 ! h264parse ! "
      "rtph264pay config-interval=-1 name=payloader ! "
...
void soup_websocket_handler (G_GNUC_UNUSED SoupServer * server,   SoupWebsocketConnection * connection, G_GNUC_UNUSED const char *path,
...
//  {"audio-priority", 0, 0, G_OPTION_ARG_STRING, &audio_priority,  "Priority of the audio stream (very-low, low, medium or high)",  "PRIORITY"},
  {"input-file", 0, 0, G_OPTION_ARG_STRING, &input_file,  "Input file",  "INPUT"},
  {NULL},
};

...
*/
    ...
    let mut acceptor = openssl::ssl::SslAcceptor::mozilla_intermediate(openssl::ssl::SslMethod::tls()).unwrap();
    acceptor.set_private_key_file("key.pem", openssl::ssl::SslFiletype::PEM).unwrap();
    acceptor.set_certificate_file("cert.pem", openssl::ssl::SslFiletype::PEM).unwrap();
